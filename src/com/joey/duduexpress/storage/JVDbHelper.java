package com.joey.duduexpress.storage;
...
public class JVDbHelper {
    ...
    public final static String SQL_NAME = "xiaowei.db";
     ...
     * @return
     */
    public boolean insertDevice(JVDevice device, String accountId) {
        // 插入新的数据之前, 先干掉旧的数据
        deleteDevice(device, accountId);
        ...
        try {
            ...
            MyLog.d("");
            database.execSQL(
                    "insert into device (id,data,accountId) values(?,?,?)",
                    new Object[]{
                            device.getGid(), data, accountId
                    });
            database.close();
            return true;
        } catch (Exception e) {
     ...
     * @return
     */
    public boolean deleteDevice(JVDevice device, String accountId) {
        SQLiteDatabase db = mSqlHelper.getWritableDatabase();
        try {
            String sqlStr = String.format(
                    "delete from device where id='%s' and accountId='%s'",
                    device.getGid(), accountId);
     ...
     * @return
     */
    public boolean updateDevice(JVDevice device, String accountId) {
        return insertDevice(device, accountId);
    }

    public boolean clearDevice(String accountId) {
        SQLiteDatabase db = mSqlHelper.getWritableDatabase();
        try {
            db.execSQL("delete from device where accountId='" + accountId + "'");
            return true;
        } catch (Exception e) {
            MyLog.e("error = ", e.getMessage());
            e.printStackTrace();
    ...
    public Serializable[] getList(String table, String columnKey,
        ...
        if (cursor != null) {
            MyLog.i("cursor length is" + cursor.getCount() + "position = " + cursor.getPosition());
    ...
    private class SQLiteHelper extends SQLiteOpenHelper {
        ...
        public final static String CREATE_DEVICE_TABLE = "create table if not exists device ("
